...
Status lifecycle:
  DRAFT
    ...
    → APPROVED                POST /damage/:id/review-amount action=approve
                                → calls proxy POST /api/damage-workflow
                                  Step 1: equipment → under_repair
                                  Step 2: payment-service → Stripe checkout for renter
                                  Step 3: RabbitMQ → SMS to renter
...
def _fetch_rental(rental_id: int) -> dict[str, Any]:
    """Look up rental to cache equipment_id and renter_id on claim creation."""
    try:
...
def _start_damage_workflow(claim: DamageClaim) -> None:
    """
    Calls camunda-proxy POST /api/damage-workflow to orchestrate:
      Step 1: equipment-service → under_repair
      Step 2: payment-service  → Stripe Checkout for renter
      Step 3: RabbitMQ         → SMS notification to renter
    """
    try:
        with httpx.Client(base_url=PROXY_URL, timeout=15.0) as c:
            resp = c.post("/api/damage-workflow", json={
            ...
            if resp.is_success:
                print(f"[damage-claim] Proxy workflow started for claimId={claim.id}", flush=True)
            else:
                print(f"[damage-claim] Proxy returned error: {resp.text[:200]}", flush=True)
    except Exception as exc:
        print(f"[damage-claim] Could not reach proxy: {exc}", flush=True)
...
def submit_claim(body: ClaimCreate, db: Session = Depends(get_db)):
    """Create a new claim in DRAFT status. Caches equipment_id and renter_id from rental."""
...
def list_pending_staff(db: Session = Depends(get_db)):
    """
    Returns all claims needing staff attention:
      PENDING_STAFF_REVIEW    — staff approves/rejects AI result
      PENDING_STAFF_APPROVAL  — owner submitted amount, staff must review
    """
    rows = (
...
def submit_amount(claim_id: str, body: SubmitAmountBody, db: Session = Depends(get_db)):
    ...
    Allowed from PENDING_OWNER_AMOUNT or AMOUNT_REJECTED (re-submit after staff rejection).
    Moves to PENDING_STAFF_APPROVAL.
    """
    cid = _parse_claim_id(claim_id)
    row = db.query(DamageClaim).filter(DamageClaim.id == cid).first()
    if not row:
...
def review_amount(claim_id: str, body: ReviewAmountBody, db: Session = Depends(get_db)):
    ...
    Staff Step 2 — review the amount the owner entered.
      approve → APPROVED + starts damage workflow (equipment under_repair, Stripe, SMS)
